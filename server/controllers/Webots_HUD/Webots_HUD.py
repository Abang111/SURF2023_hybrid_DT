...
def draw_hud(recieve_speed):
    ...
    nowPosition_veh=gps.getValues()
    
    dddis=math.sqrt((nowPosition_veh[0]-(-312))*(nowPosition_veh[0]-(-312))+(nowPosition_veh[1]-(-133))*(nowPosition_veh[1]-(-133)))
    dddis=round(dddis/1000,1)
    dddis_str=f"{dddis}km"
    
    
    dddis2=math.sqrt((nowPosition_veh[0]-(198))*(nowPosition_veh[0]-(198))+(nowPosition_veh[1]-(-308))*(nowPosition_veh[1]-(-308)))
    dddis2=round(dddis/1000,1)
    dddis_str2=f"{dddis}km"


    
    
    if nowPosition_veh[0] <-312:
        arrow_draw.arrow_right(draw, hud_display_width, hud_display_height)  
        navigation_info.navigation_info_right(draw, hud_display_width, hud_display_height, dddis_str)


    if nowPosition_veh[0] >=-312:
        arrow_draw.arrow_up(draw, hud_display_width, hud_display_height)  
        navigation_info.navigation_info_up(draw, hud_display_width, hud_display_height, dddis_str2)    
...
def main(config_file=None):
    ...
    while driver.step() != -1:
        ...
        # 定义目标文件夹路径
        HUD_folder = os.path.join(desktop_path, "Webots_HUD")

        # 如果目标文件夹不存在，则创建它们
        os.makedirs(HUD_folder, exist_ok=True)
    ...
    # 将数据分别写入不同的文本文件
        for file_name, data in file_paths.items():
            ...
            file_path = os.path.join(HUD_folder, file_name)
