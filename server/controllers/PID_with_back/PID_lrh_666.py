...
def get_filtered_speed(speed):
    ...
    if len(get_filtered_speed.previousSpeeds) > 150:  # keep only 80 values
        ...
        if in_road:
            ...
            if index is not None and index < len(cur_rdwp) - 1:
                ...
                if vehicle_side == "left":
                    driver.setSteeringAngle(angle_diff*3+0.01)   
                ...
                if frontDistance > speed / 3.6:
                    ...
                    driver.setCruisingSpeed(speed)
                #print(speed)
                #print(f"frontDistance is: {frontDistance}")
                #print(f"frontRange is: {frontRange}")

                speedDiff = driver.getCurrentSpeed() - speed
                if speedDiff > 0:
                    driver.setBrakeIntensity(min(speedDiff / (speed + 1), 1))
                else:
