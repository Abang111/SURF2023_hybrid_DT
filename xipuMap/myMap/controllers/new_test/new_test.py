...
def draw_hud(recieve_speed):
    ...
    # 设置文本颜色和大小
    font_size = 10
    font = ImageFont.truetype("C:/Users/SimCCAD/Desktop/my_project/font/Pixelarial11/PIXEAB__.TTF", font_size)
   ...
   #  vehicle = driver.getFromDef("BmwX5")  # 将"vehicle_name"替换为你的车辆模型名称
   #
   #  if vehicle is not None:
   #      # 设置车辆ID
   #      vehicle_id = 123  # 设置你想要的ID
   #      vehicle.setId(vehicle_id)
   #  ego_vehicle_id = vehicle_id # 替换为您的车辆ID
   #
   #  # 获取本车的位置信息
   #  ego_vehicle_position = traci.vehicle.getPosition()
   #  ego_vehicle_y = ego_vehicle_position[1]
   #
   #  # 获取车辆当前所在车道的ID
   #  current_lane = traci.vehicle.getLaneID(vehicle_id)
   #  current_lane_index = traci.lane.getLaneIndex
   #  left_lane_id = None
   #  right_lane_id = None
   #
   #  # 获取左右车道的ID
   #  if current_lane_index > 0:
   #      left_lane_id = current_lane - 1
   #  if current_lane < 2:
   #      right_lane_id = current_lane_index + 1
   #
   #  # 根据指定的车道方向过滤车辆
   #  if left_lane_id:
   #      # 获取左侧车道上的车辆ID列表
   #      left_lane_vehicles = traci.lane.getLastStepVehicleIDs(left_lane_id)
   #      for veh_id in left_lane_vehicles:
   #          veh_position = traci.vehicle.getPosition(veh_id)
   #          veh_y_L = veh_position[1]  # 获取车辆在Y轴上的位置
   #
   #  elif right_lane_id:
   #      # 获取右侧车道上的车辆ID列表
   #      right_lane_vehicles = traci.lane.getLastStepVehicleIDs(right_lane_id)
   #      for veh_id in right_lane_vehicles:
   #          veh_position = traci.vehicle.getPosition(veh_id)
   #          veh_y_R = veh_position[1]  # 获取车辆在Y轴上的位置
   #
   #  distance_left = ego_vehicle_y - veh_y_L
   #  if distance_left<=5:
   #  #左车道后车
   #
   #         rect_left = 64
   #         rect_top = 190 + 6 * distance_left  # 根据 distance_left 调整正方形的 top
   #         rect_right = 75
   #         rect_bottom = 200 + 6 * distance_left  # 根据 distance_left 调整正方形的 bottom
   #         draw.rectangle([rect_left, rect_top, rect_right, rect_bottom], fill=(0, 0, 255))
   #  # 当 distance_left 达到0时，调整正方形的位置
   #  if distance_left == 0:
   #         rect_top = 190
   #         rect_bottom = 200
   #
   ...
   #
   #  #左车道前车
   #  if veh_y_L > ego_vehicle_y:
   #     distance_left = veh_y_L- ego_vehicle_y
   #
   #
   #     rect_left = 64
   #     rect_top = 160 + 6 * distance_left  # 根据 distance_left 调整正方形的 top
   #     rect_right = 75
   #     rect_bottom = 170 + 6 * distance_left  # 根据 distance_left 调整正方形的 bottom
   #
   #  # 当 distance_left 达到0时，调整正方形的位置
   #  if distance_left == 0:
   #         rect_top = 190
   ...
   #
   # #右车道后车
   #  distance_right = ego_vehicle_y - veh_y_R
   #  if distance_right <= 5:
   #      rect_left = 104
   #      rect_top = 190 + 6 * distance_left  # 根据 distance_left 调整正方形的 top
   #      rect_right = 119
   ...
   #      distance_right -= 1
   #  #右车道前车
   #  if veh_y_R > ego_vehicle_y:
   #      distance_right = veh_y_R - ego_vehicle_y
   #      rect_left = 104
   #      rect_top = 160 + 6 * distance_left  # 根据 distance_left 调整正方形的 top
   #      rect_right = 119
   #      rect_bottom = 170 + 6 * distance_left  # 根据 distance_left 调整正方形的 bottom
...
speed_file = open("C:/Users/SimCCAD/Desktop/FYP.txt", "w")
rear_distance_file = open("C:/Users/SimCCAD/Desktop/rear_distance.txt", "w")
front_distance_file = open("C:/Users/SimCCAD/Desktop/front_distance.txt", "w")
current_position_file = open("C:/Users/SimCCAD/Desktop/current_position.txt", "w")
...
speed_file.close()
rear_distance_file.close()
front_distance_file.close()
current_position_file.close()

# traci.start(["sumo-gui", "-c", "C:/Users/guoka/Desktop/my_project/worlds/map_net/map.sumocfg"])
speed_file = open("C:/Users/SimCCAD/Desktop/FYP.txt", "w")
