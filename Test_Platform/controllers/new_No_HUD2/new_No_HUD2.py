...
def main(config_file=None):
    ...
    # 定义目标文件夹路径
    NO_HUD_folder = os.path.join(desktop_path, "Webots_NO_HUD2")
    ...
    # 定义文件路径，添加时间后缀
    file_paths = {
        f"position_{current_date_time}.txt": "当前位置",
        f"speed_{current_date_time}.txt": "当前速度",
        f"nowFront_{current_date_time}.txt": "前车车距",
        f"nowRear_{current_date_time}.txt": "后车车距"
    }

    # 检查并创建新文件，如果文件不存在则写入创建时的日期和时间
    creation_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    for file_name in file_paths.keys():
        file_path = os.path.join(NO_HUD_folder, file_name)
        if not os.path.exists(file_path):
            with open(file_path, 'w', encoding='utf-8') as file:
                file.write(f"文件创建时间: {creation_time}\n")
    ...
    while driver.step() != -1:
        ...
        for file_name, data in {
            f"position_{current_date_time}.txt": nowPosition,
            f"speed_{current_date_time}.txt": nowSpeed,
            f"nowFront_{current_date_time}.txt": nowFront,
            f"nowRear_{current_date_time}.txt": nowRear
        }.items():
