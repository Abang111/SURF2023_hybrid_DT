...
def get_options():
    ...
    # optParser.add_option("--order", type="int", dest="order", default=1,
    #                      help="tells TraCI to give the current client the given position in the execution order.")
    optParser.add_option("--order", type="int", dest="order", default=1,
                help="Specifies the execution order for the current TraCI client. "
                    "TraCI supports multiple clients connected to a SUMO simulation. "
                    "The 'order' parameter determines the priority of execution for this client. "
                    "Clients with a lower 'order' value are executed first. "
                    "For example, the server (SUMO) can be configured with '--order=1' to ensure it updates "
                    "global simulation states (e.g., traffic lights, vehicle generation) before any client-side "
                    "commands are executed. "
                    "Clients, such as a Python control script, can be configured with '--order=2' to execute "
                    "their specific tasks (e.g., controlling vehicles, collecting data) after the server has "
                    "updated the simulation state. "
                    "By default, the order is set to 2, making it suitable for client applications."
                )
...
arguments = [sumoBinary, "-c", configurationFile, "--start",
             "--quit-on-end=true", "--step-length=" + str(options.step / 1000.0),"--num-clients", "2", "--remote-port", str(options.port)]
...
if options.sumoArguments != "":
 ...
 # If the order of the client is 1, the SUMO on this device is opened as the terminal of the two clients.
 # If the Order of the device is not 1, the SUMO terminal is not opened, but the Webots client is selected to connect
 # to the SUMO server opened by the device with order 1.
...
controller.run(options.port, options.disableTrafficLights, directory,
               options.step,options.order, options.rotateWheels, options.maxVehicles,
 ...
 # The same is true of SUMO's termination. If the order of this client is not 1, then its SUMO terminal will be never
