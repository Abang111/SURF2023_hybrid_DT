...
def write_to_file(id, value):
    ...
    # 定义文件路径，存储在SUMO文件夹下，并命名为speed.txt
    file_path = os.path.join(sumo_folder_path, "speed.txt")
...
class SumoSupervisor (Supervisor):
    ...
    def get_vehicles_position(self, id, subscriptionResult, step, xOffset, yOffset,
        ...
        # Skip processing for Webots-specific vehicle "webotsVehicle0"
    ...
    def get_webots_vehicles_position(self, id, step, xOffset, yOffset,
        ...
        """Compute the new desired position and orientation for all the vehicles controlled by Webots client but uploaded to SUMO.."""
    ...
    def update_vehicles_position_and_velocity(self, step, rotateWheels):
        ...
        for i in range(1, self.vehicleNumber):
            if self.vehicles[i].inUse:
                ...
                spe=math.sqrt(velocity[0]**2 + velocity[1]**2 + velocity[2]**2)

                write_to_file(self.vehicles[i].node.getDef(),spe)  
    ...
    def update_webots_vehicles_position_and_velocity(self, xOffset, yOffset, step, rotateWheels):
        ...
        roll = 0.0
        pitch = 0.0
        sumoPos = self.traci.vehicle.getPosition("webotsVehicle1")
        pos = [sumoPos[0] + xOffset, sumoPos[1] + yOffset, height]
        sumoAngle = self.traci.vehicle.getAngle("webotsVehicle1")
        angle = -math.pi * sumoAngle / 180
        vehicleLength = self.traci.vehicle.getLength("webotsVehicle1")
        pos[0] += 0.5 * vehicleLength * math.sin(angle)
        pos[1] -= 0.5 * vehicleLength * math.cos(angle)
        dx = math.cos(angle)
        dz = -math.sin(angle)
    ...
    def run(self, port, disableTrafficLight, directory, step, order, rotateWheels,
        ...
        p = self.webotsVehicles[0].get_position(xOffset, yOffset)
        ...
        while self.step(step) >= 0:
            ...
            tttime=self.getTime()
            if tttime>0 and tttime<40:
                self.traci.vehicle.setSpeed("1",10)
            if tttime>40 and tttime<65:
                self.traci.vehicle.setSpeed("1",20)
            if tttime>65:
                self.traci.vehicle.setSpeed("1",10)
