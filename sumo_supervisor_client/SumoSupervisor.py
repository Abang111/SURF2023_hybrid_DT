...
class SumoSupervisor(Supervisor):
    ...
    def get_initial_vehicles(self):
        ...
        for i in range(1, self.vehiclesLimit):
            ...
            if node:
                ...
                break
        print("webots vehicle number", self.webotsVehicleNumber)
    ...
    def get_vehicle_index(self, id, generateIfneeded=True):
        ...
        for i in range(1, self.vehicleNumber):
    ...
    def get_vehicles_position(self, id, subscriptionResult, step, xOffset, yOffset,
        ...
        # Skip processing for Webots-specific vehicle "webotsVehicle1"
        if id == "webotsVehicle1":
            return
    ...
    # 
    def get_webots_vehicles_position(self, id, step, xOffset, yOffset,
                                     maximumLateralSpeed, maximumAngularSpeed):
        """Compute the new desired position and orientation for all the vehicles controlled by Webots server but uploaded to SUMO."""
        if id == "webotsVehicle1":
            height = 0.4
            roll = 0.0
            pitch = 0.0
            sumoPos = self.traci.vehicle.getPosition(id)
            sumoAngle = self.traci.vehicle.getAngle(id)
            pos = [sumoPos[0] + xOffset, sumoPos[1] + yOffset, height]
            angle = -math.pi * sumoAngle / 180
            dx = math.cos(angle)
            dz = -math.sin(angle)
            yaw = -math.atan2(dx, dz)
            # correct position (origin of the car is not the same in Webots / sumo)
            vehicleLength = self.traci.vehicle.getLength(id)
            pos[0] += 0.5 * vehicleLength * math.sin(angle)
            pos[1] -= 0.5 * vehicleLength * math.cos(angle)
    ...
    def update_webots_vehicles_position_and_velocity(self, xOffset, yOffset, step, rotateWheels):
        """Update the actual position (using angular and linear velocities) of the specific vehicle ("webotsVehicle0") transferred from SUMO."""
        height = 0.4
        roll = 0.0
        pitch = 0.0
        sumoPos = self.traci.vehicle.getPosition("webotsVehicle0")
        pos = [sumoPos[0] + xOffset, sumoPos[1] + yOffset, height]
        sumoAngle = self.traci.vehicle.getAngle("webotsVehicle0")
        angle = -math.pi * sumoAngle / 180
        vehicleLength = self.traci.vehicle.getLength("webotsVehicle0")
        pos[0] += 0.5 * vehicleLength * math.sin(angle)
        pos[1] -= 0.5 * vehicleLength * math.cos(angle)
        dx = math.cos(angle)
        dz = -math.sin(angle)
        ...
        self.vehicles[0].currentPos = pos
        self.vehicles[0].currentRot = rot
        self.vehicles[0].translation.setSFVec3f(self.vehicles[0].currentPos)
        self.vehicles[0].rotation.setSFRotation(self.vehicles[0].currentRot)
        self.vehicles[0].targetAngles = [roll, pitch, yaw]
        for j in range(0, 3):
            diffAngle = self.vehicles[0].currentAngles[j] - self.vehicles[0].targetAngles[j]
            diffAngle = (diffAngle + 2 * math.pi) % (2 * math.pi)
            if (diffAngle > math.pi):
                diffAngle -= 2 * math.pi

        if rotateWheels:
            angularVelocity = [0, self.vehicles[0].speed / self.vehicles[0].wheelRadius, 0]
            for wheelAngularVelocity in self.vehicles[0].wheelsAngularVelocity:
                wheelAngularVelocity.setSFVec3f(angularVelocity)


    def update_webots_vehicles(self, xOffset, yOffset):
        """Update the position of all the vehicles controlled by Webots in SUMO."""
        for i in range(1, self.webotsVehicleNumber):
            if self.webotsVehicles[i].is_on_road(xOffset, yOffset, self.maxWebotsVehicleDistanceToLane, self.net):
                self.webotsVehicles[i].update_position(self.getTime(), self.net, self.traci, self.sumolib, xOffset,
                                                       yOffset)
            else:
        ...
        for i in range(0, self.trafficLightNumber):
            ...
            for j in range(0, self.trafficLights[id].lightNumber):
                ...
                if trafficLightNode is not None:
                    self.trafficLights[id].trafficLightRecognitionColors[j] = trafficLightNode.getField(
                        "recognitionColors")
    ...
    def run(self, port, disableTrafficLight, directory, step, order, rotateWheels,
        ...
        try:
            ...
            # Host is set to a specific IP, port is dynamically assigned based on input
            traci.init(port, numRetries=10, host="localhost of server")

            print('traci init successful...')

            # Set the execution order for this TraCI client (lower values execute first).
            # Typically, server uses order=1 for updates, and client use order=2 to act on the updated state.
            traci.setOrder(order)

        except:
            sys.exit(
                "Unable to connect to SUMO, please make sure any previous instance of SUMO is closed.\n You can try changing SUMO port using the '--port' argument.")
        ...
        if useDisplay:
            ...
            if display is not None:
                ...
                self.sumoDisplay = SumoDisplay(display, displayZoom, view, directory, displayRefreshRate,
                                               displayFitSize,
                                               self.traci)
        ...
        while self.step(step) >= 0:
            ...
            #if "webotsVehicle0" in idList:
            self.update_webots_vehicles_position_and_velocity(xOffset, yOffset,step, rotateWheels)
